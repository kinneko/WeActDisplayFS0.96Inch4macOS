...
文字の描画には、macOS に標準搭載されている `Menlo` フォントを指定しています。

```xml
font-family="Menlo"
```

そのため、macOS では追加のフォントをインストールする必要はありません。

現在のスクリプトでは、文字サイズを以下のように固定しています。

- 1行表示: `18px`
- 2行表示: `16px`

表示する文字列が長い場合は、160×80ピクセルの表示領域からはみ出して切れることがあります。その場合は、スクリプト内の `font-size` を小さくしてください。

また、日本語など `Menlo` に含まれない文字を指定した場合は、macOS 側で別のフォントへフォールバックして描画されることがあります。このため、文字の種類によっては表示位置や文字幅が英数字の場合と異なることがあります。

Linux 環境へ移植して ImageMagick などでSVGを描画する場合、`Menlo` がインストールされていないことがあります。その場合は、Linux 環境に存在するフォントへ `font-family` の指定を変更してください。
